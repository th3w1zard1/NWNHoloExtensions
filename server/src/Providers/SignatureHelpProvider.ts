import { SignatureHelp, SignatureInformation, SignatureHelpParams } from "vscode-languageserver/node";
...
export default class SignatureHelpProvider extends Provider {
  ...
  private providerHandler(params: SignatureHelpParams): SignatureHelp | null {
    const {
      textDocument: { uri },
      position,
    } = params;
    ...
    if (!liveDocument || !document) return null;
    ...
    if (!tokensArray || !this.server.tokenizer.isInScope(tokensArray, position, LanguageScopes.functionCall)) return null;
    ...
    const validActiveParameter = activeParameter !== null ? activeParameter : undefined;
    ...
    if (functionComplexToken) {
      const helpItem = SignatureHelpBuilder.buildFunctionItem(functionComplexToken as FunctionComplexToken, validActiveParameter);
  
      if (helpItem) {
        return helpItem as SignatureHelp;
      }
    }
